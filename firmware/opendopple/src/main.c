...
volatile /* const */ u_int32_t oid = OID_CODE, seed = 0xFFFFFFFF;
u_int32_t seq = 0;
...
void dopplegang() {
  ...
  // Who are you?  You, it so happens, are me.
  oid = ((uint32_t)packet[11] & 0xff) | 
    ((uint32_t)packet[10] & 0xff) << 8 |
    ((uint32_t)packet[9] & 0xff) << 16 | 
    ((uint32_t)packet[8] & 0xff) << 24;

  // If we didn't dup the sequence number, we'd
  // be too easy to mine after the fact.
  seq = ((uint32_t)packet[7] & 0xff) | 
    ((uint32_t)packet[6] & 0xff) << 8 |
    ((uint32_t)packet[5] & 0xff) << 16 | 
    ((uint32_t)packet[4] & 0xff) << 24;
